...
void joystickHandler(const sensor_msgs::msg::Joy::ConstSharedPtr joy)
{
  ...
  // FIX: L2 pressed enables autonomy mode
  if (joy->axes[2] <= -0.1) {
    autonomyMode = true;
  }
  // FIX: Only disable autonomy when user intentionally operates the joystick
  // (L2 released AND physical movement detected). This prevents the joystick
  // from accidentally disabling autonomy when just connected or with slight drift.
  else if (joySpeedRaw > 0.05) {
    autonomyMode = false;
  }
...
void speedHandler(const std_msgs::msg::Float32::ConstSharedPtr speed)
{
  ...
  if (autonomyMode && speedTime - joyTime > joyToSpeedDelay && joySpeedRaw == 0) {
    if (speed->data > 0) {
      ...
      // FIX: When /speed topic has no publisher (data=0) or sends 0,
      // use autonomySpeed as default to ensure the vehicle moves in autonomy mode
